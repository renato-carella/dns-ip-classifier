...
def create_training_set(n_row, ip_addresses):
    ...
    for i in range(n_row):
        ...
        for j in range(4):
            training_set[i][j] = result[j]
        training_set[i][4] = asn
    print("...Done")
    ...
    for i in range(n_row):
        ...
        asn = retrieve_as(ip)
        for j in range(4):
            training_set[n_row + i][j] = result[j]
        training_set[n_row + i][4] = asn
    print("...Done")
...
def create_test_set(n_regulars, n_outliers, n_train, scan):
    ...
    for i in range(n_regulars):
        ...
        asn = retrieve_as(scan[n_train + i])
        for j in range(4):
            test_set[i][j] = result[j]
        test_set[i][4] = asn
    print("...Done")
    ...
    for i in range(n_outliers):
        ...
        asn = retrieve_as(ip)
        for j in range(4):
            test_set[n_regulars + i][j] = result[j]
        test_set[n_regulars + i][4] = asn
    print("...Done")
    return test_set
